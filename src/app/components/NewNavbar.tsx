...
useEffect(() => {
    ...
    return(
        <nav className={`w-full h-20 flex justify-center top-0 shadow pl-10 md:pl-36 fixed z-30 bg-neutral-300/80 border border-white rounded-xl backdrop-blur-md  ${isVisible ? 'translate-y-0 transition-transform duration-300 ease-in-out' : '-translate-y-full transition-transform duration-300 ease-in-out'}`} 
       >
            ...
            <div className="h-full w-1/2 grid grid-cols-4 grid-rows-1 items-center">
                <div className="col-span-1 row-span-1 w-full ml-9">
                    <Link href="/about" className=" text-sm font-bold hidden md:flex text-neutral-800 hover:underline hover:underline-offset-8 md:text-lg">Projects</Link>
                </div>
